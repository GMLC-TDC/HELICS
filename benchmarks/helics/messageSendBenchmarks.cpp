    ...
    for (auto _ : state) {
        ...
        if (singleCore) {
            wcore = helics::CoreFactory::create(
                cType, std::string("--federates=") + std::to_string(wcore_fed_count));
        }
...
BENCHMARK_CAPTURE(BM_sendMessage, ipcMultiCore, core_type::IPC)
    ...
    ->Ranges(
        {{1, 1 << 11},
         {1,
          1 << 9}}) // msg count has a much bigger effect on time taken (increasing size had no noticeable effect on times)
    ->Iterations(1)
    ->Unit(benchmark::TimeUnit::kMillisecond)
    ->UseRealTime();
...
BENCHMARK_CAPTURE(BM_sendMessage, udpMultiCore, core_type::UDP)
    ...
    ->Ranges(
        {{1, 1 << 15},
         {1,
          1}}) // msg size of 65536 bytes causes error/terminate, though somewhere about 8K the benchmark time drops from several ms to <1ms
    ->Ranges(
        {{1, 1},
         {1,
          1 << 6}}) // msg count has a bigger effect on time taken (increasing size had minimal effect on times); larger sizes/counts did seem to result in hanging, maybe an important packet was lost
    ->Iterations(1)
    ->Unit(benchmark::TimeUnit::kMillisecond)
    ->UseRealTime();
#endif
