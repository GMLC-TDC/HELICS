...
class EchoHub_c:
    ...
    def run(self, parties, vFed):
        ...
        """
        logging.info(
            'echo hub - starting the execution of the helics functions')
        self.vFed = vFed
        ...
        logging.info(
            'echo hub - finished the execution of the helics functions')
...
class EchoLeaf_c:
    ...
    def run(self, parties, vFed):
        ...
        """
        logging.info(
            'echo leaf - starting the execution of the helics functions')
        self.vFed = vFed
        self.parties = parties
        if not self.readyToRun:
            self.make_ready(self.vFed)
        ...
        self._main_loop(self.vFed)
        logging.info(
            'echo leaf - finished the execution of the helics functions')
...
def BMecho_multiCore(cTypeString, federates):
    ...
    broker = h.helicsCreateBroker(
        cTypeString, "brokerf", initString)
    wcore = h.helicsCreateCore(
...
def create_bm_dictionary(name, federate_count, core_type,
                         real_time, cpu_time, threads):
    ...
    if name == 'BMecho_singleCore':
        bm_dict = {
            'name': '{}/{}/iterations:1/real_time'.format(
                name, federate_count),
            'run_name': '{}/{}/iterations:1/real_time'.format(
                name, federate_count),
            'run_type': 'iteration',
            'repetitions': 1,
            'repetitions_index': 1,
        ...
        bm_dict = {
            'name': '{}/{}Core/{}/real_time'.format(
                name, core_type, federate_count),
            'run_name': '{}/{}Core/{}/real_time'.format(
                name, core_type, federate_count),
            'run_type': 'iteration',
            'repetitions': 1,
            'repetitions_index': 1,
...
def create_output_file(benchmark, output_path, filename, date, bm_dicts):
    ...
    build_flags = str(build_flags_dict.get('base')) + '\\' + 'py{}'.format(
        build_flags_dict.get('py_version'))
    ...
    string += 'CPU MODEL: {}\n'.format(
        cpuinfo.get_cpu_info().get('brand'))
...
def _auto_run(args):
    ...
    for i in range(0, args.power):
        ...
        single_dict = create_bm_dictionary(
            'BMecho_singleCore', 2**i, 'singleCore',
            single_real_time, single_cpu_time, 1)
        ...
        inproc_dict = create_bm_dictionary(
            'BMecho_multiCore', 2**i, 'inproc',
            inproc_real_time, inproc_cpu_time, 1)
        ...
        zmq_dict = create_bm_dictionary(
            'BMecho_multiCore', 2**i, 'zmq',
            zmq_real_time, zmq_cpu_time, 1)
        ...
        zmqss_dict = create_bm_dictionary(
            'BMecho_multiCore', 2**i, 'zmqss',
            zmqss_real_time, zmqss_cpu_time, 1)
        ...
        udp_dict = create_bm_dictionary(
            'BMecho_multiCore', 2**i, 'udp',
            udp_real_time, udp_cpu_time, 1)
        ...
        benchmarks.append(
            [single_dict, inproc_dict, zmq_dict, zmqss_dict, udp_dict])
    ...
    create_output_file(
        'echoBenchmark', args.output_path,
        'bm_echo_pyResults{}_{}'.format(
            datetime.date.today(), str(run_id)), date_time,
        benchmarks)
