  ...
  public:
    ...
    void initialize (const std::string &coreName, int index, int maxIndex, int message_size)
    {
       ...
       // fi.setFlagOption (helics_flag_restrictive_time_policy);
        ...
        ept->setDefaultDestination ("ept_" + std::to_string ((index_ == maxIndex_ - 1) ? 0 : (index_ + 1)));
        initialized = true;
    }
...
static void BM_ringMessage2_singleCore (benchmark::State &state)
{
    for (auto _ : state)
    {
        ...
        {
            links[ii].initialize (wcore->getIdentifier (), ii, feds,static_cast<int>(state.range(0)));
        }
...
BENCHMARK (BM_ringMessage2_singleCore)->Unit (benchmark::TimeUnit::kMillisecond)->UseRealTime ()->Iterations (3)->Range(8,8<<6);
...
static void BM_ringMessage_multiCore (benchmark::State &state, core_type cType)
{
    for (auto _ : state)
    {
        ...
        {
            threadlist[ii] = std::thread ([&](RingTransmitMessage &link) { link.run ([&brr]() { brr.wait (); }); },
                                          std::ref (links[ii + 1]));
        }
