...
# Versions
The 1.3.1 branch is the most stable.  The 2.0 version in HELICS_2_0 and soon to be in develop is currently in active development and nearly fully functional with only a few new features left to test and verify, Anyone looking to use HELICS for new projects or existing projects should start looking at transitioning to the 2.0.0 releases.  1.3.X will only be updated for bug fixes, and once the final 2.0.0 comes out, all existing code and examples will be transitioned to the 2.0 API and code.    
