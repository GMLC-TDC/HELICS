...
int main(int argc, char* argv[])
{
    ...
    SWIGTYPE_p_void GasPubPth =
        h.helicsFederateRegisterGlobalTypePublication(vfed, "GasThermalPower", "double", "");
    SWIGTYPE_p_void GasPubPthMax =
        h.helicsFederateRegisterGlobalTypePublication(vfed, "GasThermalPowerMin", "double", "");
    ...
    h.helicsFederateSetTimeProperty(vfed,
                                    (int)HelicsProperties.HELICS_PROPERTY_TIME_PERIOD,
                                    period);

    // check to make sure setting the time property worked
    double period_set =
        h.helicsFederateGetTimeProperty(vfed, (int)HelicsProperties.HELICS_PROPERTY_TIME_PERIOD);
    Console.WriteLine($ "Time period: {period_set}");

    // set max iteration at 20
    h.helicsFederateSetIntegerProperty(vfed,
                                       (int)HelicsProperties.HELICS_PROPERTY_INT_MAX_ITERATIONS,
                                       20);
    int iter_max = h.helicsFederateGetIntegerProperty(
        vfed, (int)HelicsProperties.HELICS_PROPERTY_INT_MAX_ITERATIONS);
    ...
    ostrm = new FileStream(outputfolder + "Log_gas_federate.txt",
    ...
    for (TimeStep = 0; TimeStep < total_time; TimeStep++) {
        ...
        if (TimeStep == 0) {
            // granted_time = h.helicsFederateRequestTimeIterative(vfed, TimeStep, iter_flag, out
            // helics_iter_status);
            MappingFactory.PublishGasThermalPower(
                granted_time - 1, Iter, PthermalMax, GasPubPth, 1, GasPubPthMax);
        }
        // Set time step info
        ...
        while (IsRepeating && Iter < iter_max) {
            ...
            // get requested thermal power from connected gas plants, determine if there are
            // violations
            HasViolations = false;

            // iterative HELICS time request
            // get publication from electric federate and avoid negative Pthermal due to the
            // negative cubic function (to be checked in the electric federate)
            granted_time = h.helicsFederateRequestTimeIterative(vfed,
                                                                TimeStep,
                                                                iter_flag,
                                                                out helics_iter_status);
            double val = h.helicsInputGetDouble(SubToElectric);
            Console.WriteLine(
                String.Format("Gas-Received: Time {0} \t iter {1} \t Pthe = {2:0.0000} [MW]",
                              TimeStep,
                              Iter,
                              val));
            ...
            if (Math.Abs(Pthermal[TimeStep] - val) > 0.001) {
                ...
                Console.WriteLine(
                    String.Format("Gas-Event: Time {0} \t iter {1} \t Pthe = {2:0.0000} [MW]",
                                  TimeStep,
                                  Iter,
                                  Pthermal[TimeStep]));
            } else {
                if (GasLastVal.Count > 1) {
                    if (Math.Abs(GasLastVal[GasLastVal.Count - 2] -
                                 GasLastVal[GasLastVal.Count - 1]) > 0.001) {
                        HasViolations = true;
                    }
                } else {
                    HasViolations = true;
            ...
            granted_time = h.helicsFederateRequestTimeIterative(vfed,
                                                                ...
                                                                out helics_iter_status);
            Console.WriteLine(
                $ "Granted time: {granted_time - 1}, iteration status: {helics_iter_status}");
            // using an offset of 1 on the granted_time here because HELICS starts at t=1 and SAInt
            // starts at t=0
            MappingFactory.PublishGasThermalPower(granted_time - 1,
                                                  Iter,
                                                  Pthermal[TimeStep],
                                                  GasPubPth,
                                                  (PthermalMax - val),
    ...
    using(FileStream fs = new FileStream(outputfolder + "TimeStepInfo_gas_federate.txt",
                                         FileMode.OpenOrCreate,
                                         FileAccess.Write))
    {
    ...
    using(FileStream fs = new FileStream(outputfolder + "NotConverged_gas_federate.txt",
                                         FileMode.OpenOrCreate,
                                         FileAccess.Write))
    {
        using(StreamWriter sw2 = new StreamWriter(fs))
    ...
    using(FileStream fs = new FileStream(outputfolder + "PthermalAndPthermalNew.txt",
                                         FileMode.OpenOrCreate,
                                         FileAccess.Write))
    {
        using(StreamWriter sw2 = new StreamWriter(fs))
        {
            sw2.WriteLine(
                "TimeStep \t PthermalOld[MW]\t PthermalNew[MW] \tPthermalMin[MW]\t PthermalMax[MW]");
            for (int t = 0; t < total_time; t++) {
                sw2.WriteLine(String.Format("{0}\t\t{1:0.00}\t\t{2:0.00}\t\t{3:0.00}\t\t{4:0.00}",
                                            t,
                                            PthermalOld[t],
                                            PthermalNew[t],
                                            PthermalMin,
