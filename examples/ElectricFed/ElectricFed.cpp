...
int main(int argc, char* argv[])
{
    ...
    SWIGTYPE_p_void ElectricPub =
        h.helicsFederateRegisterGlobalTypePublication(vfed, "ElectricPower", "double", "");
    SWIGTYPE_p_void SubToGas = h.helicsFederateRegisterSubscription(vfed, "GasThermalPower", "");
    SWIGTYPE_p_void SubToGasMin =
        h.helicsFederateRegisterSubscription(vfed, "GasThermalPowerMin", "");
    ...
    for (TimeStep = 0; TimeStep < total_time; TimeStep++) {
        // non-iterative time request here to block until both federates are done iterating the last
        // time step
        ...
        // Initial publication of thermal power request equivalent to PGMAX for time = 0 and iter =
        // 0;
        if (TimeStep == 0) {
        ...
        while (IsRepeating && Iter < iter_max) {
            ...
            HasViolations = MappingFactory.SubscribeToGasThermalPower(
                granted_time - 1, Iter, P[TimeStep], SubToGas, ElecLastVal);

            // get currently required thermal power
            // double HR = 5 + 0.5 * P[TimeStep] - 0 * P[TimeStep] * P[TimeStep];
            // double ThermalPower = HR / 3.6 * P[TimeStep]; //Thermal power in [MW]; //
            // eta_th=3.6/HR[MJ/kWh]
            ...
            granted_time = h.helicsFederateRequestTimeIterative(vfed,
                                                                ...
                                                                iter_flag,
                                                                out helics_iter_status);
            std::cout << $ "Granted time: {granted_time-1},  Iteration status: {helics_iter_status}"
                      << std::endl;
