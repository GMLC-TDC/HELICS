...
class HelicsCppFederate {
    private:
        ...
        vector<fedEp> eps;
        void runBroker(void) {
            broker = new helicscpp::Broker(string("zmq"), string("broker"), string("-f 1 --loglevel=ERROR"));
        ...
        void registerWithHelics (void) {
            ...
            fedLog->log(LogLevel::INFO, string("Successfull Registration with HELICS Broker.\n"));
        ...
        void disconnectFromHelics(void) {
            ...
            if(federate->getCurrentMode() != HelicsFederateState::HELICS_STATE_FINALIZE || federate->getCurrentMode() != HelicsFederateState::HELICS_STATE_ERROR) {
                federate->finalize();
            }
            broker->disconnect();
            ...
            while(currentTime <= endTime) {
                if(federate->getCurrentMode() == HelicsFederateState::HELICS_STATE_EXECUTION) {
                    ...
                    if(currentTime < endTime) {
                        ...
                        fedLog->log(LogLevel::INFO, format("Cpp Federate Status: {}% complete.", completionStatus));
                    } else {
                        fedLog->log(LogLevel::INFO, string("Cpp Federate Status: Simulation complete.\n"));
                        break;
                    }
                } else {
                    break;
...
string usage(string programName) {
    ...
    string rv;
    usageStream.clear();
    usageStream << "Usage: " << programName << " federateConfigFile duration (optional)publishIterations\n";
    ...
    usageStream << "\t\tpublishIterations: An option argument for setting the number of times the federate will publish/send for each publication/endpoint in a single timestep. The defualt is 1." << endl;
    rv = usageStream.str();
    usageStream.clear();
    return rv;
}
...
int main(int argc, char* argv[]) {
    ...
    if( argc != 3) {
        stringstream errMsg;
        errMsg.clear();
        errMsg << "Incorrect number arguments provided. helicsCppFederate takes 2 positional arguments.\n";
