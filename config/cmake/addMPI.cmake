...
if(NOT MPI_C_FOUND)
    if(MSVC) # the cmake find MPI doesn't completely work for visual studio 2017
        ...
        if(NOT MPI_CXX_COMPILER)
            # message(STATUS "not mpi cxx compiler") For building MPI programs the selected Visual
            # Studio compiler is used, namely cl.exe. So there is no need to set a specific MPI
            # compiler.
...
if(MPI_C_FOUND AND NOT TARGET MPI::MPI_C)
    ...
    if(MPI_C_LIBRARIES)
        set_property(
            TARGET MPI::MPI_C APPEND PROPERTY INTERFACE_LINK_LIBRARIES "${MPI_C_LIBRARIES}"
        )
    endif()
    if(MPI_C_LINK_FLAGS)
        string(STRIP "${MPI_C_LINK_FLAGS}" trimmed_mpi_c_flags)
        set_property(
            TARGET MPI::MPI_C APPEND PROPERTY INTERFACE_LINK_LIBRARIES "${trimmed_mpi_c_flags}"
        )
    endif()
    ...
    set_target_properties(
        MPI::MPI_C PROPERTIES INTERFACE_COMPILE_OPTIONS "${MPI_C_COMPILE_OPTIONS}"
    )
    set_target_properties(
        MPI::MPI_C PROPERTIES INTERFACE_COMPILE_DEFINITIONS "${MPI_C_COMPILE_DEFINITIONS}"
    )
endif()
if(MPI_CXX_FOUND AND NOT TARGET MPI::MPI_CXX)
    ...
    if(MPI_CXX_LIBRARIES)
        set_property(
            TARGET MPI::MPI_CXX APPEND PROPERTY INTERFACE_LINK_LIBRARIES "${MPI_CXX_LIBRARIES}"
        )
    endif()
    if(MPI_CXX_LINK_FLAGS)
        string(STRIP "${MPI_CXX_LINK_FLAGS}" trimmed_mpi_cxx_flags)
        set_property(
            TARGET MPI::MPI_CXX APPEND PROPERTY INTERFACE_LINK_LIBRARIES "${trimmed_mpi_cxx_flags}"
        )
    endif()
    set_target_properties(
        MPI::MPI_CXX
    ...
    set_target_properties(
        MPI::MPI_CXX PROPERTIES INTERFACE_COMPILE_OPTIONS "${MPI_CXX_COMPILE_OPTIONS}"
    )
    set_target_properties(
        MPI::MPI_CXX PROPERTIES INTERFACE_COMPILE_DEFINITIONS "${MPI_CXX_COMPILE_DEFINITIONS}"
    )
