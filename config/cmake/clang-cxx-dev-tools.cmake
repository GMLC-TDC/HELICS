...
file(
    ...
    src/helics/shared_api_library/*.[ch]pp
    src/helics/shared_api_library/*.[ch]
    src/helics/shared_api_library/internal/*.h
)
...
set(ALL_APPLICATION_FILES ${ALL_APPLICATION_API_FILES} ${ALL_APP_FILES}
                          ${ALL_SHARED_API_FILES})
...
if(CLANG_FORMAT)
    add_custom_target(
        clang-format-test
        COMMAND ${CLANG_FORMAT} -i -style=file ${ALL_TEST_FILES}
    )

    add_custom_target(
        clang-format-application
        COMMAND ${CLANG_FORMAT} -i -style=file ${ALL_APPLICATION_FILES}
    )

    add_custom_target(
        clang-format-core
        COMMAND ${CLANG_FORMAT} -i -style=file ${ALL_CORE_FILES}
    )

    add_custom_target(
        clang-format-examples
        COMMAND ${CLANG_FORMAT} -i -style=file ${ALL_EXAMPLE_FILES}
    )

    add_custom_target(
        clang-format-all
        DEPENDS
            clang-format-test clang-format-application clang-format-core
            clang-format-examples
    )
endif()
