...
# this function is to get a nicely formatted build string for later reference in a human
# readable format that has some utility for automation. the resulting format would be
# empty if the build matches a tag exactly, and otherwise <branch_name>-<git
# hash>[-dirty]  if dirty is present there are uncommitted changes in the repo called
# like git_version_describe(${PROJECT_SOURCE_DIR} result)  then result will contain the
# description string
...
function(git_version_describe source_path result)
    ...
    if(GIT_FOUND)
        execute_process(
            COMMAND
                ${GIT_EXECUTABLE} -C ${source_path} --work-tree ${source_path} describe
                --tags
        ...
        if(TAGGING_ERROR)
            execute_process(
                COMMAND
                    ${GIT_EXECUTABLE} -C ${source_path} --work-tree ${source_path}
                    describe --all --tags --dirty --long
                OUTPUT_VARIABLE TAG_DESCRIPTION
                ERROR_VARIABLE TAGGING_ERROR
                OUTPUT_STRIP_TRAILING_WHITESPACE
            )
        ...
        if(NOT TAGGING_ERROR)
            ...
            if(tag_length GREATER 16)
                ...
                if(last_dash_loc GREATER 0)
                    ...
                    execute_process(
                        COMMAND
                            ${GIT_EXECUTABLE} -C ${source_path} --work-tree
                            ${source_path} describe --all --tags --dirty
                    ...
                    if(hash_loc LESS 0)
                        ...
                        if(dirty_loc GREATER 0)
                            string(SUBSTRING ${TAG_DESCRIPTION2} 0 ${dirty_loc}
                                             tag_desc
                            )
