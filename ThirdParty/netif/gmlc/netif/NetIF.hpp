...
/**
 * a helper function to get a list of all interface adapters using OS system calls. the returned pointer from addrs
 * must be freed with a call to freeAddresses.
 * @param family type of adapter addresses to get on Windows; one of AF_INET (IPv4), AF_INET6 (IPv6), or AF_UNSPEC
 * (both)
 * @param[out] addrs pointer to a pointer to be filled in with the address of the allocated address structure; must
 * be freed when done by calling freeAddresses
 ...
 * @return the next interface/adapter in the addresses list (that matches the family given for non-Windows
 * systems), or nullptr if there is no next address
 */
