...
-   The exported C++ shared library has been heavily modified to only include functions in the public API, and is not the recommended way to link with HELICS directly in a C++ program.  HELICS::helics-shared target is now available for linking through cmake.  If libraries were previously linking with the installed static library this is a BREAKING Change.  those previously linking with the C++ shared library may also need modifications.  Changes include:
