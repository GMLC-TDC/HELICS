...
int main(int, char**)
{
    ...
    std::cout << "created broker \"" << broker->getIdentifier()
              << "\"\n"
              << "broker is connected: " << std::boolalpha
              << broker->isConnected() << std::endl;

    std::mt19937_64 gen(std::random_device{}());
    std::uniform_real_distribution<double> dist(
        0.0,std::nextafter(10.0,std::numeric_limits<double>::max()));
    ...
    for (unsigned tstep = 0; tstep < num_tsteps; ++tstep)
    {
        ...
        // Output to log file
        ofs << std::setw(10) << std::right << this_time
            << std::setw(10) << std::right << this_value << std::endl;

        sendPublication(
            fed,ValueSetter(thisTime, id, this_value));
