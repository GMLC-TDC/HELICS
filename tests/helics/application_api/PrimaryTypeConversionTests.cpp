...
TEST (type_conversion_tests, vectorNorm_tests)
{
    ...
    EXPECT_EQ (vectorNorm (cv{c (-3.0, -4.0)}), 5.0);
    EXPECT_EQ (vectorNorm (cv{c (-3.0, -4.0), c (-3.0, -4.0), c (-3.0, -4.0), c (-3.0, -4.0), c (-3.0, -4.0)}),
               std::sqrt (25.0 * 5.0));
}
...
TEST (type_conversion_tests, string_converstion_tests)
{
    ...
    EXPECT_TRUE (checkTypeConversion1 (vstr, std::vector<std::complex<double>>{std::complex<double> (val, 0.0)}));
...
TEST (type_conversion_tests, namedType_tests)
{
    ...
    EXPECT_TRUE (getTypeFromString (typeid (char).name ()) == data_type::helics_string);
    EXPECT_TRUE (getTypeFromString (typeid (std::complex<double>).name ()) == data_type::helics_complex);
...
TEST (type_conversion_tests, namedpoint_conversion_tests)
{
    ...
    EXPECT_TRUE (checkTypeConversion1 (vp, std::vector<std::complex<double>>{std::complex<double> (val, 0.0)}));
    ...
    EXPECT_TRUE (checkTypeConversion1 (vp2, std::vector<std::complex<double>>{std::complex<double> (3.0, -4.0)}));
