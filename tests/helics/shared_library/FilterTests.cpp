...
class filter_simple_type_tests : public ::testing::TestWithParam<const char *>, public FederateTestFixture
{
};
...
TEST_F (filter_tests, core_filter_reg)
{
  ...
  CE (auto sourceFilter1 =
        helicsCoreRegisterFilter (core1, helics_filter_type::helics_filter_type_delay, "core1SourceFilter", &err));
  ...
  EXPECT_TRUE (core1IsConnected != 0);
...
TEST_F (filter_tests, multi_clone_test)
{
  ...
  if (res)
  {
    ...
    if (res)
    {
      ...
      EXPECT_STREQ (m2.dest, "dest");
      EXPECT_EQ (m2.length, static_cast<int64_t> (data2.size ()));
    }
  }
...
INSTANTIATE_TEST_SUITE_P (filter_tests, filter_simple_type_tests, ::testing::ValuesIn (core_types_simple));
