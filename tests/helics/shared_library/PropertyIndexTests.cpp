...
TEST(prop_tests, flagprops) {
    ...
    EXPECT_EQ(helicsGetFlagIndex("IGNORE_TIME_MISMATCH_WARNINGS"), helics_flag_ignore_time_mismatch_warnings);
    EXPECT_EQ(helicsGetFlagIndex("SOURCE_ONLY"),
    ...
    EXPECT_EQ(helicsGetFlagIndex("WAIT_FOR_CURRENT_TIME_UPDATE"), helics_flag_wait_for_current_time_update);
    EXPECT_EQ(helicsGetFlagIndex("RESTRICTIVE_TIME_POLICY"),
