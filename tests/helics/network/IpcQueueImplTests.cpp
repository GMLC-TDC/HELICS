...
BOOST_AUTO_TEST_SUITE(IpcQueueImpl_tests, *utf::label("ci"))
...
BOOST_AUTO_TEST_CASE(multithreaded_tests)
{
    ...
    auto cons = [&]() {
        ...
        while ((res)) {
            ...
            if (res == 0) {  // make an additional sleep period so the producer can catch up
...
BOOST_AUTO_TEST_CASE(push_full_mem)
{
    ...
    auto* queue = new (memblock2.get())
        helics::ipc::detail::IpcBlockingPriorityQueueImpl(memblock.get(), 4096);
    std::vector<unsigned char> data(500, 'a');
    BOOST_CHECK(queue->try_push(data.data(), 420));  // this would go into the pull
    BOOST_CHECK(queue->try_push(data.data(), 420));  // this would go into the push
    BOOST_CHECK(queue->try_push(data.data(), 420));  // this would go into the push
...
BOOST_AUTO_TEST_CASE(multithreaded_tests3_pop_mem)
{
    ...
    auto* qn = new (memblock2.get())
        helics::ipc::detail::IpcBlockingPriorityQueueImpl(memblock.get(), 1048576);
