...
TEST(brokers, force_override_ci_skip_nocov)
{
   ...
   // EXPECT_TRUE(brk3->isConnected());
    // NOTE(PT) the test for connection on the third broker is ureliable due to the nature of ZMQ reapers.
    // the test for connection on the first broker should work reliably.  
    EXPECT_FALSE(brk->isConnected());
