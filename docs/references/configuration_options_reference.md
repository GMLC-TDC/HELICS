...
- `--logbuffer` - Specify that the last 10 log messages should be stored in a circular buffer to be retrieved via a query
- `--logbuffersize=` - Specify that the last X log messages should be stored in a circular buffer to be retrieved via a query (only 1 can be used of logbuffersize, and logbuffer)
...
### `logbuffersize` | `log_buffer_size` | `logBufferSize` [10]
