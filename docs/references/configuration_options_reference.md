...
- `--federates=` - The minimum Number of federates expected to join a federation though the broker. Equivalent to `-f`
...
- `--children=` - The minimum number of child object the broker should expect before allowing entry to initializing state.
- `--subbrokers=` - The minimum number of child object the broker should expect before allowing entry to initializing state. Same as `--children` but might be clear in some case with multilevel hierarchies.
- `--brokerkey=` - A broker key to use for connections
