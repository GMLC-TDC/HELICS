...
- `--dynamic` - Allows the operation of dynamic federations where one or more federates join after others have entered executing mode.
...
Profiling output is appended (rather than overwriting) the output file specified by the `profiler` option.
