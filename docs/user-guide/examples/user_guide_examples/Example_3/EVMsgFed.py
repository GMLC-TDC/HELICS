...
def get_new_EV(numEVs):
    ...
    lvl3 = np.random.poisson(np.random.normal(20, np.random.uniform(0.05, 0.25)), 1)
...
if __name__ == "__main__":
    ...
    for EV in EVs:
        ...
        end_EVsoc.append(h.helicsFederateRegisterEndpoint(fed, end_name, "double"))

        dest_name = f"EVController_federate/EV{EV}.soc"
        enddest_EVsoc.append(h.helicsEndpointSetDefaultDestination(end_EVsoc[EV - 1], dest_name))
        ...
        for j in range(0, len(enddest_EVsoc)):
            ...
            if h.helicsEndpointHasMessage(end_EVsoc[j]):
                ...
                if int(instructions) == 1:
                    addenergy = charge_rate[(EVlist[j] - 1)] * (
                        update_interval / 3600
                    )  # time_since_last_msg[j]
                    ...
                    currentsoc[j] = np.random.uniform(0.05, 0.5)
                    # print('new soc: ',currentsoc[j])
                # 3. Send SOC
                # end_name = str(h.helicsEndpointGetName(end_EVsoc[j]))
                destination_name = str(h.helicsEndpointGetDefaultDestination(end_EVsoc[j]))
                ...
                print(
                    ...
                    t,
                    " with endpoint ",
                    str(h.helicsEndpointGetDefaultDestination(end_EVsoc[j])),
                )
