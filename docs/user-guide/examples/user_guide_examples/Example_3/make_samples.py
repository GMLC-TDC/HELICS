...
def make_federate(cmd):
    runner = {"federates": [], "name": "Generated by make samples"}

    for k, v in cmd.items():
        federate = {"directory": ".", "exec": v, "host": "broker", "name": k, "loglevel": 3}
        ...
        for i in range(int(samples)):
            ...
            with open(output_path + "/" + runner_name, "w") as h_runner:
                runner_dict.update(
                    {
                        "broker_"
                        + str(i): "helics_broker --federates=2 --loglevel=5 --type=tcpss --port "
                        + str(port)
                    }
                )
                runner_dict.update(
                    {
                        "EVMsgFed_"
                        + str(i): "python3 EVMsgFed.py --port {} --seed {}".format(
                            str(port), str(seed)
                        )
                    }
                )
                runner_dict.update(
                    {
                        "EVController_"
                        + str(i): "python3 EVControllerMsgFed.py --port {}".format(str(port))
                    }
                )
