...
```sh
HelicsBroker --CoreType=zmq --port=20200 &
HelicsBroker --CoreType=zmq --port=20400 &
```
...
It will also generate brokers as needed so the `HelicsBroker` does not need to be restarted for every run.
...
```sh
HelicsBroker --CoreType=zmq --key=my_broker_key
```
