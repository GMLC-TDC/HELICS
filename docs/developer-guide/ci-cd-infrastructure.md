...
To avoid long build queues, commits to `main` will be tested with more configurations,
...
marked with `[daily]` are only run daily on the `main` branch. A tag of `[main]` indicates that
