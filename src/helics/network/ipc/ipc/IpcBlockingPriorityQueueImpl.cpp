...
namespace ipc {
    namespace detail {
        ...
        /** default constructor*/
        IpcBlockingPriorityQueueImpl::IpcBlockingPriorityQueueImpl(unsigned char* dataBlock,
                                                                   int blockSize):
            queueSize(sizeAlign8(blockSize, 0.4)),
            prioritySize(sizeAlign8(blockSize, 0.2)), pushData(dataBlock, queueSize),
            pullData(dataBlock + queueSize, queueSize),
            priorityData(dataBlock + 2 * queueSize, prioritySize), dataBlock_(dataBlock),
            dataSize(blockSize)
        {
        }
        ...
        */
        ...
        void IpcBlockingPriorityQueueImpl::push(const unsigned char* data, int size)
        {
            ...
            while (true) {
                scoped_lock<interprocess_mutex> pushLock(
        ...
        int IpcBlockingPriorityQueueImpl::push(std::chrono::milliseconds timeout,
                                               ...
                                               int size)
        {
            ...
            while (true) {
                ...
                conditionLock.unlock();
                bool conditionMet =
                    condition_empty.timed_wait(pushLock,
                                               boost::posix_time::microsec_clock::universal_time() +
                                                   boost::posix_time::milliseconds(
                                                       timeout.count()));  // now wait
                if (pushData.isSpaceAvailable(size)) {
                    pushData.push(data, size);
                    return size;
                }
        ...
        val the value to push on the queue
        */
        void IpcBlockingPriorityQueueImpl::pushPriority(const unsigned char* data,
                                                        int size)  // forwarding reference
        {
            ...
            while (true) {
                ...
                if (size > priorityData.capacity() - 8) {
                    throw(std::invalid_argument(
                        "data size is greater than priority buffer capacity"));
                }
        ...
        val the value to push on the queue
        */
        int IpcBlockingPriorityQueueImpl::pushPriority(std::chrono::milliseconds timeout,
                                                       const unsigned char* data,
                                                       int size)  // forwarding reference
        {
            if (try_pushPriority(data, size)) {
            ...
            while (true) {
                ...
                conditionLock.unlock();
                bool conditionMet =
                    condition_empty.timed_wait(pullLock,
                                               boost::posix_time::microsec_clock::universal_time() +
                                                   boost::posix_time::milliseconds(
                                                       timeout.count()));  // now wait
                if (priorityData.isSpaceAvailable(size)) {
        ...
        int IpcBlockingPriorityQueueImpl::try_pop(unsigned char* data, int maxSize)
        {
            ...
            if (pullData.empty()) {
                ...
                if (!pushData.empty()) {  // on the off chance the queue got out of sync
                    ...
                    pushLock.unlock();  // we can free the push function to accept more elements
                                        // after the swap call;
                    pullData.reverse();
            ...
            if (pullData.empty()) {
                scoped_lock<interprocess_mutex> pushLock(m_pushLock);  // second PushLock
                if (!pushData.empty()) {  // this has the potential for slow operations
                    pushData.swap(pullData);
                    scoped_lock<interprocess_mutex> conditionLock(m_conditionLock);
                    if (queueFullFlag) {
                        queueFullFlag = false;
        ...
        int IpcBlockingPriorityQueueImpl::pop(std::chrono::milliseconds timeout,
                                              unsigned char* data,
                                              int maxSize)
        {
            auto val = try_pop(data, maxSize);
