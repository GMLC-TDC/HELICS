...
HELICS_SHARED_DEPRECATED std::shared_ptr<Broker> findBroker (const std::string &brokerName) { return nullptr; }

/** try to find a joinable broker of a specific type*/
HELICS_SHARED_DEPRECATED std::shared_ptr<Broker> findJoinableBrokerOfType (core_type type) { return nullptr; }
...
HELICS_SHARED_DEPRECATED bool registerBroker (const std::shared_ptr<Broker> &broker) { return false; }
