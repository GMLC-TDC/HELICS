...
void helicsFederateGlobalError(helics_federate fed, int error_code, const char* errorString)
{
    auto* fedObj = getFed(fed, nullptr);
    if (fedObj == nullptr) {
        return;
    ...
    try {
        fedObj->globalError(error_code, AS_STRING(errorString));
    }
    // LCOV_EXCL_START
    catch (...) {
    }
...
void helicsFederateLocalError(helics_federate fed, int error_code, const char* errorString)
{
    auto* fedObj = getFed(fed, nullptr);
    if (fedObj == nullptr) {
        return;
    ...
    try {
        fedObj->localError(error_code, AS_STRING(errorString));
    }
    // LCOV_EXCL_START
    catch (...) {
    }
