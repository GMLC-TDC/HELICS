...
#include "helics.h"
	...
	HELICS_Export helicsStatus helicsSendMessageRaw(helics_endpoint_id_t endpoint, const char *dest, const char *data, uint64_t len);

	HELICS_Export helicsStatus helicsSendEventRaw( helics_endpoint_id_t endpoint, const char *dest, const char *data, uint64_t len, helics_time_t time);

	HELICS_Export helicsStatus helicsSendMessage( helics_endpoint_id_t endpoint, message_t *message);

	HELICS_Export helicsStatus helicsSubscribe( helics_endpoint_id_t endpoint, const char *name, const char *type);
	/** check if the federate has any outstanding messages*/
	HELICS_Export bool helicsFederateHasMessage(helics_message_federate_id_t fedID);
	/* check if a given endpoint has any unread messages*/
	HELICS_Export bool helicsEndpointHasMessage(helics_endpoint_id_t endpoint);

	/**
	* Returns the number of pending receives for the specified destination endpoint.
	*/
	HELICS_Export  uint64_t helicsFederateReceiveCount(helics_message_federate_id_t fedID);
	/**
	* Returns the number of pending receives for all endpoints of particular federate.
	*/
	HELICS_Export uint64_t helicsEndpointReceiveCount(helics_endpoint_id_t endpoint);

	/** receive a packet from a particular endpoint
	@param[in] endpoint the identifier for the endpoint
	@return a message object*/
	message_t * helicsEndpointGetMessage(helics_endpoint_id_t endpoint);
	...
	message_t * helicsFederateGetMessage(helics_message_federate_id_t fedID);
