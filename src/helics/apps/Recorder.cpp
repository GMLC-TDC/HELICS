...
Recorder::Recorder (const std::string &appName, CoreApp &core, const FederateInfo &fi) : App (appName, core, fi)
{
  fed->setFlagOption (helics_flag_observer);
}
...
void Recorder::writeTextFile (const std::string &filename)
{
  ...
  for (auto &v : points)
  {
    if (v.first)
    {
      outFile << static_cast<double> (v.time) << "\t\t" << subscriptions[v.index].getTarget () << '\t' << v.value
    ...
    if (sub.isUpdated ())
    {
      ...
      if (verbose)
      {
        ...
        else
        {
          ...
          {
            valstr =
              fmt::format ("[{}:{}]value {}=block[{}]", currentTime, iteration, sub.getTarget (), val.size ());
          }
          else
          {
  ...
  for (auto &ept : endpoints)
  {
    while (ept.hasMessage ())
    {
      ...
      if (verbose)
      {
        ...
        {
          messstr = fmt::format ("[{}]message from {} to {}:: size {}", currentTime, mess->source, mess->dest,
                                 mess->data.size ());
        }
...
void Recorder::runTo (Time runToTime)
{
  ...
  if (!mapfile.empty ())
  {
    ...
    for (auto &stat : vStat)
    {
      ...
      //    stat.lastVal
      //        << '\n';
      fmt::print (out, "{}\t{}\t{}\t{}\n", stat.key, stat.cnt, static_cast<double> (stat.time), stat.lastVal);
    }
    out.flush ();
  }
...
std::shared_ptr<helicsCLI11App> Recorder::buildArgParserApp ()
{
  ...
  auto capture_group =
    app->add_option_group ("capture_group", "Options related to capturing publications, endpoints, or federates");
  capture_group
