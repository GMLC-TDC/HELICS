...
Tracer::Tracer (const std::string &appName, CoreApp &core, const FederateInfo &fi) : App (appName, core, fi)
{
  fed->setFlagOption (helics_flag_observer);
}
  ...
  if (cloneEndpoint)
  {
    while (cloneEndpoint->hasMessage ())
    {
      ...
      if (printMessage)
      {
        ...
        if (mess->data.size () < 50)
        {
          messstr = fmt::format ("[{}]message from {} to {}::{}", currentTime, mess->source, mess->original_dest,
                                 mess->data.to_string ());
        }
        else
        {
