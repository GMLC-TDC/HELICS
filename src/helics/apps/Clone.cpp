...
Clone::Clone (const std::string &appName, const std::shared_ptr<Core> &core, const FederateInfo &fi)
    : App (appName, core, fi)
{
  fed->setFlagOption (helics_flag_observer);
}
...
void Clone::setFederateToClone (const std::string &federateName) { captureFederate = federateName; }
