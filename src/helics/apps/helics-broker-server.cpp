...
int main (int argc, char *argv[])
{
  ...
  auto term = cmdLine
                .add_subcommand ("term", "helics_broker_server term will start a broker server "
  ...
  try
  {
    ...
    {
      auto brokerServer = std::make_unique<helics::apps::BrokerServer> (cmdLine.remaining_for_passthrough ());
...
void terminalFunction (std::vector<std::string> args)
{
  ...
  */
  termProg.add_subcommand ("help", "display the help")->callback ([&termProg]() { termProg.helics_parse ("-?"); });
  /*
  std::string target;
  std::string query;
