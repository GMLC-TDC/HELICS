...
void TcpComms::queue_rx_function ()
{
  ...
  while (!server->isReady ())
  {
    ...
    {  // If we failed and we are on an automatically assigned port number,  just try a different port
      server->close ();
      ++PortNumber;
      server = helics::tcp::TcpServer::create (ioctx->getBaseContext (), localTargetAddress,
                                          ...
                                          std::shared_ptr<TcpConnection> &brokerConnection)
{
  ...
  try
  {
    brokerConnection = makeConnection (ioctx->getBaseContext (), brokerTargetAddress, std::to_string (brokerPort),
                                       maxMessageSize, connectionTimeout);
    int retries = 0;
    ...
    if (PortNumber <= 0)
    {
      ...
      tcp::endpoint brk;
      brokerConnection->async_receive (rx.data (), 128, [this, &rx](const std::error_code &error, size_t bytes) {
        if (!error)
        {
      ...
      if (hasBroker)
      {
        ...
        catch (const std::system_error &se)
        {
          if (se.code () != asio::error::connection_aborted)
          {
            ...
            {
              logError (std::string ("broker send 0 ") + actionMessageType (cmd.action ()) + ':' + se.what ());
            }
          }
        }
      ...
      if (rt_find != routes.end ())
      {
        ...
        catch (const std::system_error &se)
        {
          if (se.code () != asio::error::connection_aborted)
          {
            if (!isDisconnectCommand (cmd))
            {
              logError (std::string ("rt send ") + std::to_string (rid.baseValue ()) + "::" + se.what ());
            }
          }
        }
      }
        ...
        if (hasBroker)
        {
          ...
          catch (const std::system_error &se)
          {
            if (se.code () != asio::error::connection_aborted)
            {
              if (!isDisconnectCommand (cmd))
              {
                logError (std::string ("broker send") + std::to_string (rid.baseValue ()) + " ::" + se.what ());
              }
            }
          }
        }
