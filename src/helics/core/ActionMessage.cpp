...
const std::string nullStr = "unknown";

std::string actionMessageType (action_message_def::action_t action)
{
    static const std::map<action_message_def::action_t, std::string> actionStrings{
      // priority commands
    ...
    auto fnd = actionStrings.find (action);
    if (fnd != actionStrings.end ())
    {
        return fnd->second;
    }
    return nullStr;
