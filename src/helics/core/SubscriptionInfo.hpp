...
class SubscriptionInfo
{
  public:
      struct dataRecord
      {
          ...
          dataRecord() = default;
          dataRecord(Time recordTime, std::shared_ptr<const data_block > recordData):time(recordTime),data(std::move(recordData))
          {}
          dataRecord(Time recordTime, int recordIndex, std::shared_ptr<const data_block > recordData) :time(recordTime),index(recordIndex), data(std::move(recordData))
          {}
      };
