...
{
private:
    mutable boost::interprocess::interprocess_mutex m_pushLock;  //!< lock for operations on the pushElements vector
    void *push = nullptr;   
    int32_t *pushIndices;
    size_t pushSize = 0;
    mutable boost::interprocess::interprocess_mutex m_pullLock;  //!< lock for elements on the pullLock vector
    void *pull = nullptr;
    int32_t *pullIndices;
    size_t pullSize = 0;
    mutable boost::interprocess::interprocess_mutex conditionLock  //!< lock for the empty and full Flag
    bool queueEmptyFlag{ true };  //!< flag indicating the queue is empty
    bool queueFullFlag{ true };
    // the condition variable should be keyed of the conditionLock
    boost::interprocess::interprocess_condition condition_empty;  //!< condition variable for notification of new data
    boost::interprocess::interprocess_condition condition_full;  //!< condition variable for notification of available space
    void *data1=nullptr;
    void *data2=nullptr;
    size_t data1Size = 0;
    size_t data2Size = 0;
    void *dataPriority=nullptr;
    size_t dataPrioritySize = 0;
    
public:
    /** default constructor*/
    IpcBlockingPriorityQueue() = default;
    ...
    ~IpcBlockingPriorityQueue() { clear(); }
    ...
    */
    explicit IpcBlockingPriorityQueue(size_t capacity)
    ...
    IpcBlockingPriorityQueue(IpcBlockingPriorityQueue &&bq) noexcept
    ...
    IpcBlockingPriorityQueue &operator= (BlockingPriorityQueue &&sq) noexcept
    {
        std::lock_guard<std::mutex> pullLock(m_pullLock);  // first pullLock
        std::lock_guard<std::mutex> pushLock(m_pushLock);  // second pushLock
    ...
    IpcBlockingPriorityQueue(const BlockingPriorityQueue &) = delete;
    IpcBlockingPriorityQueue &operator= (const BlockingPriorityQueue &) = delete;
...
template <typename T>
stx::optional<T> IpcBlockingPriorityQueue<T>::try_pop()
{
    std::lock_guard<std::mutex> pullLock(m_pullLock);  // first pullLock
    if (!priorityQueue.empty())
    {
...
template <typename T>
bool IpcBlockingPriorityQueue<T>::empty() const
{
