...
void CoreBroker::processPriorityCommand (ActionMessage &&command)
{
  ...
  case CMD_REG_FED:
  {
    ...
    else
    {
      ...
      LOG_CONNECTIONS (global_broker_id_local, getIdentifier (),
                       fmt::format ("registering federate {}({}) on route {}", command.name,
                                    global_fedid.baseValue (), route_id.baseValue ()));
    }
  }
  break;
  case CMD_REG_BROKER:
  {
    ...
    {
      _brokers.back ().global_id = global_broker_id (static_cast<global_broker_id::base_type> (_brokers.size ()) -
                                                     1 + global_broker_id_shift);
    ...
    else
    {
      ...
      if (ept == nullptr)
      {
        if (isRootc)
        {
          ...
          {
            unknownHandles.addDestinationFilterLink (command.name, command.getString (targetStringLoc));
          }
          else
          {
            unknownHandles.addSourceFilterLink (command.name, command.getString (targetStringLoc));
          }
        }
        else
        {
...
void CoreBroker::processBrokerConfigureCommands (ActionMessage &cmd)
{
  ...
  case UPDATE_LOGGING_CALLBACK:
    ...
    else
    {
      ...
      {
        auto M =
          stx::any_cast<std::function<void(int, const std::string &, const std::string &)>> (std::move (*op));
      ...
      case CMD_ADD_NAMED_INPUT:
        ...
        if (!command.getStringData ().empty ())
        {
          ...
          if (pub == nullptr)
          {
            ...
            auto &apub = handles.addHandle (command.source_id, command.source_handle, handle_type::publication,
      ...
      case CMD_ADD_NAMED_ENDPOINT:
        ...
        if (!command.getStringData ().empty ())
        {
          ...
          if (filt == nullptr)
          {
            ...
            auto &afilt =
              handles.addHandle (command.source_id, command.source_handle, handle_type::filter, std::string (),
...
void CoreBroker::addInput (ActionMessage &m)
{
  ...
  auto &inp =
    handles.addHandle (m.source_id, m.source_handle, handle_type::input, m.name, m.getString (0), m.getString (1));
      ...
      if (unknownHandles.hasRequiredUnknowns ())
      {
        ...
        unknownHandles.processRequiredUnknowns (
          [this, &eMiss](const std::string &target, char type, global_handle handle) {
            ...
            {
            case 'p':
              eMiss.payload = fmt::format ("Unable to connect to required publication target {}", target);
    ...
    else if (command.dest_id == parent_broker_id)
    {
      ...
      if (brk != nullptr)
      {
        LOG_CONNECTIONS (parent_broker_id, getIdentifier (),
                         fmt::format ("got disconnect from {}({})", brk->name, command.source_id.baseValue ()));
...
std::string CoreBroker::getNameList (std::string gidString) const
{
  ...
  {
    auto info =
      handles.findHandle (global_handle (global_federate_id (val[index]), interface_handle (val[index + 1])));
    if (info != nullptr)
    {
