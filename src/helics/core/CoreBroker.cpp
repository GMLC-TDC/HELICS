...
{
    auto &pub=handles.addHandle (m.source_id, m.source_handle, HANDLE_PUB, m.name, m.info ().type, m.info ().units);
...
{
    auto &ept=handles.addHandle (m.source_id, m.source_handle,  HANDLE_END, m.name, m.info ().type, m.info ().units);
