  ...
  while (!bindsuccess)
  {
    ...
    catch (const std::system_error &error)
    {
      ...
      {  // If we failed and we are on an automatically assigned port number,  just try a different port
        ...
        {
          disconnecting = true;
          logError (fmt::format ("unable to bind socket {} :{}", makePortAddress (localTargetAddress, PortNumber),
                                 error.what ()));
          socket.close ();
          setRxStatus (connection_status::error);
          return;
...
void UdpComms::queue_tx_function ()
{
  ...
  if (hasBroker)
  {
    ...
    try
    {
      udp::resolver::query query (udpnet (interfaceNetwork), brokerTargetAddress, std::to_string (brokerPort));
  ...
  while (true)
  {
    ...
    if (rid == parent_route_id)
    {
      ...
      {
        logWarning (fmt::format ("message directed to broker of comm system with no broker, message dropped {}",
    ...
    else
    {
      ...
      if (rt_find != routes.end ())
      {
        ...
        if (error)
        {
          logWarning (fmt::format ("transmit failure sending to route {}:{}", rid.baseValue (), error.message ()));
        }
      }
      else
      {
        ...
        else
        {
          ...
          {
            logWarning (std::string ("(udp) unknown route, message dropped ") + prettyPrintString (cmd));
          }
        }
      }
    }
