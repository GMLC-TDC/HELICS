...
std::vector<std::pair<int, std::string>> InterfaceInfo::checkInterfacesForIssues ()
{
  ...
  for (auto &ipt : ihandle)
  {
    ...
    for (auto &source : ipt->source_info)
    {
      ...
      {
        issues.emplace_back (helics::defs::errors::connection_failure,
                             fmt::format ("Input \"{}\" source has mismatched types: {} is not compatible with {}",
                                          ipt->key, ipt->type, std::get<1> (source)));
      }
      if ((!ipt->ignore_unit_mismatch) && (!checkUnitMatch (ipt->units, std::get<2> (source), false)))
      {
        issues.emplace_back (helics::defs::errors::connection_failure,
                             fmt::format ("Input \"{}\" source has incompatible unit: {} is not convertible to {}",
                                          ipt->key, std::get<2> (source), ipt->units));
      }
    }
  }
  ihandle.unlock ();
  ...
  for (auto &pub : phandle)
  {
    ...
    if (pub->single_destination)
    {
      ...
      {
        issues.emplace_back (helics::defs::errors::connection_failure,
                             fmt::format ("Publication {} is single source only but has more than one connection",
                                          pub->key));
      }
    }
  }
