                                              ...
                                              const std::string &type_out)
{
    ...
    auto handle =
      createBasicHandle (global_broker_id, 0, HANDLE_SOURCE_FILTER, filterName, source, type_in, type_out);
...
void CommonCore::processPriorityCommand (ActionMessage &&command)
{
    ...
    case CMD_FED_ACK:
    {
        ...
        if (fed != nullptr)
        {
            if (checkActionFlag(command, error_flag))
            {
                LOG_ERROR(0, identifier, std::string("broker responded with error for registration of ")+command.name+"\n");
            }
            else
            {
...
void CommonCore::processFilterInfo (ActionMessage &command)
{
    ...
    case CMD_NOTIFY_DST_FILTER:
    {
        ...
        {
            if ((filterInfo->destFilter->fed_id == command.source_id) && (filterInfo->destFilter->handle == command.source_handle))
            {
        ...
        if (filtFunc->hasSourceFilters)
        {
            ...
            for (auto &filt : filtFunc->sourceFilters)
            {
                if (filt->fed_id == global_broker_id)
                {
                    if (filt->cloning)
                    {
                        auto tempMessage = createMessageFromCommand(m);
                        filt->filterOp->process(std::move(tempMessage));
                    }
                    else
                    {
...
void CommonCore::processFilterReturn (ActionMessage &cmd)
{
    ...
    if (ongoingFilterProcesses[handle->fed_id].find (cmd.info ().messageID) !=
