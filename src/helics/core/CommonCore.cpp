...
local_federate_id CommonCore::registerFederate (const std::string &name, const CoreFederateInfo &info)
{
  if (!waitCoreRegistration ())
  {
    ...
    throw (RegistrationFailure ("core is unable to register and has timed out, federate cannot be registered"));
  }
...
void CommonCore::setIntegerProperty (local_federate_id federateID, int32_t property, int16_t propertyValue)
{
  if (federateID == local_core_id)
  {
    if (!waitCoreRegistration ())
    {
      throw (FunctionExecutionFailure ("core is unable to register and has timed out, property was not set"));
    }
...
void CommonCore::sendMessage (interface_handle sourceHandle, std::unique_ptr<Message> message)
{
  if (sourceHandle == direct_send_handle)
  {
    if (!waitCoreRegistration ())
    {
      throw (FunctionExecutionFailure ("core is unable to register and has timed out, message was not sent"));
    }
...
void CommonCore::setLogFile (const std::string &lfile) { setLoggingFile (lfile); }
...
void CommonCore::registerInterface (ActionMessage &command)
{
  if (command.dest_id == parent_broker_id)
  {
    ...
    case CMD_REG_FILTER:
      ...
      createFilter (global_broker_id_local, command.source_handle, command.name, command.getString (typeStringLoc),
...
void CommonCore::processFilterInfo (ActionMessage &command)
{
  ...
  if (checkActionFlag (command, destination_target))
  {
    ...
    if (!FilterAlreadyPresent)
    {
      ...
      {
        newFilter = createFilter (global_broker_id (command.source_id), command.source_handle, command.name,
