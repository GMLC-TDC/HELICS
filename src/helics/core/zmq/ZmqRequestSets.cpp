...
namespace helics
{
ZmqRequestSets::ZmqRequestSets()
{
	ctx = zmqContextManager::getContextPointer();
}
...
bool ZmqRequestSets::checkForMessages(std::chrono::milliseconds timeout)
{
	...
	auto res = std::remove_if(active_routes.begin(), active_routes.end(), [](const auto &ar) {return (ar.events == 0); });
	if (res != active_routes.end())
	{
		active_routes.erase(res, active_routes.end());
	}
	auto res2 = std::remove_if(active_messages.begin(), active_messages.end(), [](const auto &am) {return (!am.waiting); });
...
void ZmqRequestSets::checkDelayedSend()
{
	...
	if (still_waiting)
	{
		auto rem = std::remove_if(waiting_messages.begin(), waiting_messages.end(), [](const auto &wm) {return (wm.first == 0); });
