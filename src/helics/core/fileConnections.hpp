  ...
  if (conns != nullptr)
  {
    ...
    for (const auto &conn : connArray)
    {
      ...
      else
      {
        ...
        if (!pub.empty ())
        {
          addTargets (conn, "targets", [brk, &pub](const std::string &target) { brk->dataLink (pub, target); });
        }
        else
        {
          std::string ipt = getOrDefault (conn, "input", std::string ());
          addTargets (conn, "targets", [brk, &ipt](const std::string &target) { brk->dataLink (target, ipt); });
        }
      }
    }
  }
  ...
  if (filts != nullptr)
  {
    ...
    for (const auto &filt : filtArray)
    {
      ...
      else
      {
        ...
        if (!fname.empty ())
        {
          auto asrc = [brk, &fname](const std::string &ept) { brk->addSourceFilterToEndpoint (fname, ept); };
          addTargets (filt, "endpoints", asrc);
          addTargets (filt, "source_endpoints", asrc);
          addTargets (filt, "sourceEndpoints", asrc);
          auto adst = [brk, &fname](const std::string &ept) { brk->addDestinationFilterToEndpoint (fname, ept); };
