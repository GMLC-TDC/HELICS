...
namespace helics {
	...
	*/
	...
	publication_id_t ValueFederate::registerPublication(const std::string &name, const std::string &type, const std::string &units)
	{
	...
	publication_id_t ValueFederate::registerGlobalPublication(const std::string &name, const std::string &type, const std::string &units)
	{
		if (state != op_states::startup)
		{
			throw(InvalidFunctionCall("cannot call register publication after entering initialization mode"));
	...
	subscription_id_t ValueFederate::registerRequiredSubscription(const std::string &name, const std::string &type, const std::string &units)
	{
		if (state != op_states::startup)
		{
	...
	subscription_id_t ValueFederate::registerOptionalSubscription(const std::string &name, const std::string &type, const std::string &units)
	{
		if (state != op_states::startup)
		{
			throw(InvalidFunctionCall("cannot call register subscription after entering initialization mode"));
	...
	data_view ValueFederate::getValueRaw(subscription_id_t id)
	{
		return vfManager->getValue(id);
	}
	...
	bool ValueFederate::isUpdated(subscription_id_t sub_id) const
	{
		return vfManager->queryUpdate(sub_id);
	}
	...
	std::vector<subscription_id_t>  ValueFederate::queryUpdates()
	{
	...
	void ValueFederate::registerSubscriptionNotificationCallback(std::function<void(subscription_id_t,Time)>  callback)
	{
		vfManager->registerCallback(callback);
	}

	void ValueFederate::registerSubscriptionNotificationCallback(subscription_id_t id, std::function<void(subscription_id_t, Time)>  callback)
	{
		vfManager->registerCallback(id, callback);
	}

	void ValueFederate::registerSubscriptionNotificationCallback(const std::vector<subscription_id_t> &ids, std::function<void(subscription_id_t, Time)>  callback)
	{
