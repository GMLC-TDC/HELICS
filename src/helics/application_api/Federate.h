...
namespace helics {
	...
	*/
	...
	public:
		...
		bool observer = false; //!< indicator that the federate is an observer and doesn't participate in time advancement
		...
		bool uninterruptible = false;  //!< indicator that the time request cannot return something other than the requested time
	...
	*/
	...
	public:
		...
		*/
		...
		/** second part of the async process for entering initializationState call after a call to enterInitializationStateAsync
		if call any other time it will throw an invalidFunctionCall exception*/
	...
	public:
		...
		*/
		...
		std::shared_ptr<Core> getCorePointer()
		{
			return coreObject;
		}
