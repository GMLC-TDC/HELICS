...
namespace helics {
	...
	{//this constructor should only be called by child class that has already constructed the underlying federate in a virtual inheritance
		mfManager = std::make_unique<MessageFederateManager>(coreObject, getID());
	}
	...
	{
		if (getID() != mFed.getID()) //the id won't be moved, as it is copied so use it as a test if it has moved already
		{
	...
	void MessageFederate::registerKnownCommunicationPath(endpoint_id_t localEndpoint, const std::string &remoteEndpoint)
	{
		if (state == op_states::startup)
		{
	...
	*/
	...
	void MessageFederate::sendMessage(endpoint_id_t source, const std::string &dest, const data_view &data, Time sendTime)
	{
		if (state == op_states::execution)
		{
	...
	std::string MessageFederate::getEndpointName(endpoint_id_t id) const
	{
		return mfManager->getEndpointName(id);

	}

	std::string MessageFederate::getEndpointType(endpoint_id_t ep)
	{
		return mfManager->getEndpointType(ep);
	}
	...
	void MessageFederate::registerEndpointCallback(const std::vector<endpoint_id_t> &ep, std::function<void(endpoint_id_t, Time)> func)
	{
		mfManager->registerCallback(ep, func);
	}
}
