...
Time Federate::requestTimeComplete ()
{
  ...
  throw (
    InvalidFunctionCall ("cannot call finalize requestTime without first calling requestTimeIterative function"));
}
...
void Federate::registerFilterInterfacesToml (const std::string &tomlString)
{
  ...
  if (globals != nullptr)
  {
    if (globals->is<toml::Array> ())
    {
      ...
      {
        setGlobal (val.as<toml::Array> ()[0].as<std::string> (), val.as<toml::Array> ()[1].as<std::string> ());
      }
    }
    else
    {
...
void Federate::setGlobal (const std::string &valueName, const std::string &value)
{
  ...
  {
    throw (
      InvalidFunctionCall ("set set Global cannot be called on uninitialized federate or after finalize call"));
  }
}
...
void Federate::addSourceTarget (const Filter &filt, const std::string &targetEndpoint)
{
  ...
  {
    throw (
      InvalidFunctionCall ("add source target cannot be called on uninitialized federate or after finalize call"));
  }
}
