...
namespace helics
{
	...
	public:
		...
		*/
		...
		@throw invalid_argument if the publication id is invalid
		*/
		void publish(publication_id_t id, const data_block &block)
		{
			publish(id, data_view(block));
		}
		...
		@throw invalid_argument if the publication id is invalid
		*/
		void publish(publication_id_t id, const char *data)
		{
			publish(id, data_view{ data,strlen(data) });
		}
		...
		@throw invalid_argument if the publication id is invalid
		*/
		void publish(publication_id_t id, const char *data, size_t len)
		{
			publish(id, data_view{ data,len });
		}
		...
		*/
		void registerSubscriptionNotificationCallback(subscription_id_t id, std::function<void(subscription_id_t, Time)> callback);
		/** register a callback function to call when the specified subscription is updated
		@param[in] ids  the set of ids to register the callback for
		@param[in] callback the function to call
		*/
		void registerSubscriptionNotificationCallback(const std::vector<subscription_id_t> &ids, std::function<void(subscription_id_t, Time)> callback);
	private:
		/** @brief PIMPL design pattern with the implementation details for the ValueFederate*/
