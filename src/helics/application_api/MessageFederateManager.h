...
namespace helics
{
	...
	public:
		...
		*/
		...
		@param[in] callback the function to call
		*/
		void registerCallback(const std::vector<endpoint_id_t> &ids, std::function<void(endpoint_id_t, Time)> callback);
	
	private:
		
		std::unordered_map<std::string, endpoint_id_t> endpointNames;  //!< container to translate names to endpoint id's
