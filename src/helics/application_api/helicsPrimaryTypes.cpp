...
void valueExtract(const defV& dv, std::complex<double>& val)
{
    switch (dv.index()) {
        ...
        case vector_loc:  // vector
        {
            auto& vec = std::get<std::vector<double>>(dv);
            if (vec.size() == 1) {
        ...
        case complex_vector_loc: {
            auto& vec = std::get<std::vector<std::complex<double>>>(dv);
            if (!vec.empty()) {
                val = vec.front();
            }
            break;
...
void valueExtract(const defV& dv, std::vector<double>& val)
{
    ...
    switch (dv.index()) {
        ...
        {
            auto& cv = std::get<std::vector<std::complex<double>>>(dv);
...
void valueExtract(const defV& dv, std::vector<std::complex<double>>& val)
{
    ...
    switch (dv.index()) {
        ...
        case vector_loc:  // vector
        {
            auto& v = std::get<std::vector<double>>(dv);
            val.resize(v.size() / 2);
...
void valueExtract(const defV& dv, Time& val)
{
    switch (dv.index()) {
        ...
        {
            size_t index;
            auto& str = std::get<std::string>(dv);
            try {
