...
class Endpoint
{
  private:
    ...
    std::function<void(Endpoint *, Time)> localCallback; //!< the local callback function
public:
    ...
    */
    ...
    time is the time the value was updated  This callback is a notification callback and doesn't return the value
    */
    void setCallback(std::function<void(Endpoint *, Time)> callback)
    {
        localCallback = std::move(callback);
        fed->registerEndpointCallback(id, [this](endpoint_id_t, Time messageTime) {returnCallback(messageTime); });
    }
    ...
    private:
        ...
        void returnCallback(Time messageTime)
        {
            if (localCallback)
            {
                localCallback(this, messageTime);
            }
        }
