  ...
  public:
    ...
    */
        ...
        : fed (mFed), m_name (std::move (name)), m_type (std::move (type))
    {
		...
		@param[in] type a named type associated with the endpoint
		*/
	Endpoint(interface_visibility locality, MessageFederate *mFed, std::string name, std::string type = "")
		: fed(mFed), m_name(std::move(name)), m_type(std::move(type))
	{
		id = (locality == GLOBAL) ? fed->registerGlobalEndpoint(m_name, m_type) : fed->registerEndpoint(m_name, m_type);
	}
